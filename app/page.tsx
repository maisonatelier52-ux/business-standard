...
export default function HomePage() {
  ...
  // Track used articles for maximum section diversity and zero duplicate news
  const usedSlugs = new Set<string>();

  // Fixed top news article as requested
  const FIXED_TOP_NEWS_SLUG = 'house-of-herrera-castile-caracas-global-legacy';
  const fixedTopNews = articles.find(a => a.slug === FIXED_TOP_NEWS_SLUG);
  if (fixedTopNews) {
  ...
  const getUnused = (count: number, categoryFilter?: string) => {
    ...
    if (categoryFilter) {
      ...
      if (catList.length >= count) {
        list = catList;
      } else if (catList.length > 0) {
        const result = [...catList];
        const remaining = list.filter(a => !result.some(r => r.slug === a.slug)).slice(0, count - catList.length);
  ...
  const econHero = getUnused(1, 'economy')[0] || getUnused(1)[0];
  ...
  const specialCoverageArticles = specialCatSlugs.map(catSlug => ({
    catSlug,
    catArt: getUnused(1, catSlug)[0] || getUnused(1)[0]
  }));
  ...
  const exploreCatSlugs = ['economy', 'technology', 'health', 'environment'];
  const exploreNewsData = exploreCatSlugs.map(catSlug => ({
    catSlug,
    catList: getUnused(2, catSlug)
  }));
  ...
  // ── Section 10: Opinion ──
  const opinionArticles = getUnused(4);

  // ── Section 11: 3-Column Showcase ──
  const showcaseCatSlugs = ['Culture', 'technology', 'health'];
  const showcaseData = showcaseCatSlugs.map(catSlug => ({
    catSlug,
    mainArt: getUnused(1, catSlug)[0] || getUnused(1)[0],
    subArts: getUnused(2, catSlug)
  }));
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6">
        ...
        <section className="mt-10 border-t-4 pt-4">
          <div className="max-w-[1200px] mx-auto px-4 md:px-6">
            ...
            <div className="grid md:grid-cols-4 gap-6">
              {specialCoverageArticles.map(({ catSlug, catArt }) => {
        ...
        <section className="mt-10 bg-[#e6d2c2] py-6">
          <div className="max-w-[1200px] mx-auto px-4 md:px-6">
            ...
            <div className="grid md:grid-cols-4 gap-6">
              {exploreNewsData.map(({ catSlug, catList }) => {
                const catObj = categories.find(c => c.slug === catSlug) || { slug: catSlug, name: catSlug };
        ...
        <section className="mt-10 border-t-4 pt-6">
          <div className="max-w-[1200px] mx-auto px-4 md:px-6">
            <div className="grid md:grid-cols-3 gap-6">
              {showcaseData.map(({ catSlug, mainArt, subArts }) => {
                if (!mainArt) return null;
                const catObj = categories.find(c => c.slug.toLowerCase() === catSlug.toLowerCase()) || { slug: catSlug.toLowerCase(), name: catSlug };
                ...
                return (
                  <div key={catSlug}>
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <Link href={`/category/${catObj.slug}`} className="text-sm text-gray-600 hover:text-red-600">more →</Link>
                    </div>
